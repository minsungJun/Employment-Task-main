...
public partial class BoardController
{
    public bool CheckCanDestroy(BoardBlockObject boardBlock, BlockObject block)
    {
        ...
        {
            if (!boardBlock.isCheckBlock && !model.CheckBlockGroupDic.ContainsKey(checkGroupIdx)) return false;
        }
        
        int pBlockminX = model.boardWidth;
        int pBlockmaxX = -1;
        int pBlockminY = model.boardHeight;
        int pBlockmaxY = -1;

        List<BlockObject> blocks = block.dragHandler.model.blocks;
        ...
        {
            foreach (var boardBlockObj in model.CheckBlockGroupDic[checkIndex])
            {
        ...
        if (hor)
        {
            ...
            // 개별 좌표가 나갔는지 여부를 판단.
            if (pBlockminX < minX - model.blockDistance / 2 || pBlockmaxX > maxX + model.blockDistance / 2)
            {
                return false;
            }
            ...
            {
                if (horizonBoardBlocks[i].y <= model.boardHeight / 2)
                {
                    int maxY = -1;

                    for (int k = 0; k < block.dragHandler.model.blocks.Count; k++)
                    {
                        var currentBlock = block.dragHandler.model.blocks[k];
                    ...
                    for (int l = blockCheckCoors[i].Item2; l <= horizonBoardBlocks[i].y; l++)
                    {
                        ...
                        if (model.boardBlockDic.ContainsKey(key) &&
                            model.boardBlockDic[key].playingBlock != null &&
                            model.boardBlockDic[key].playingBlock.colorType != boardBlock.horizonColorType)
                        {
                            return false;
                        }
                    }
        ...
        else
        {
            ...
            if (pBlockminY < minY - model.blockDistance / 2 || pBlockmaxY > maxY + model.blockDistance / 2)
            {
                return false;
            }
            ...
            {
                //x exist in left
                if (verticalBoardBlocks[i].x <= model.boardWidth / 2)
                {
                    ...
                    for (int l = blockCheckCoors[i].Item1; l >= verticalBoardBlocks[i].x; l--)
                    {
                        ...
                        if (model.boardBlockDic.ContainsKey(key) &&
                            model.boardBlockDic[key].playingBlock != null &&
                            model.boardBlockDic[key].playingBlock.colorType != boardBlock.verticalColorType)
                        {
                            return false;
                        }
                    }
